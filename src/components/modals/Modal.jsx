...
const Modal = ({handleSubmit,closeModal, isSucessFull, handleReloadPage,}) => {
  return (
    <div className='fixed z-0 top-0 left-0 size-full flex items-center justify-center ' style={{backgroundColor: 'rgba(0,0,0,0.4)'}}>
        <div className='flex flex-col items-center gap-11 font-sen justify-center bg-white w-[95vw] sm:w-[80vw] lg:w-[50vw] rounded-lg p-8 '>
           ...
           {isSucessFull? <><SuccessSVG color={Colors.PRIMARY}/>
            ...
            <button className='mt-10 h-12 md:h-16 w-36 md:w-60 text-xl  bg-red-600 hover:bg-[rgba(252,124,124,0.9)] text-white'
                        onClick={closeModal} 
                        type='submit'>Cancel</button>
            {isSucessFull? <button className='mt-10 h-12 md:h-16 w-36 md:w-60 text-xl bg-green-600 hover:bg-[rgb(117,250,139)] text-white'
                        onClick={handleReloadPage}                             
                       // disabled={!isDataInTable}
                        type='submit'>Continue</button> : <button className='mt-10 h-12 md:h-16 w-36 md:w-60 text-xl bg-green-600 hover:bg-[rgb(117,250,139)] text-white'
